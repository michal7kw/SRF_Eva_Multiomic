...
tes_direct_file <- "SRF_Eva_integrated_analysis/output/results/direct_targets/TES_direct_targets_all_genes.csv"
if (file.exists(tes_direct_file)) {
  ...
  cat("  Neuron migration:", length(neuron_overlap), "genes (", 
  ...
  cat("  Cell proliferation:", length(prolif_overlap), "genes (",
  ...
  cat("  Apoptosis:", length(apoptosis_overlap), "genes (",
  ...
  overlap_table <- data.frame(
    ...
    PercentOfGeneSet = c(round(length(neuron_overlap)/length(neuron_migration_genes)*100, 1),
                         round(length(prolif_overlap)/length(cell_prolif_genes)*100, 1),
                         round(length(apoptosis_overlap)/length(apoptosis_genes)*100, 1)),
    PercentOfTES = c(round(length(neuron_overlap)/length(tes_direct_genes)*100, 1),
