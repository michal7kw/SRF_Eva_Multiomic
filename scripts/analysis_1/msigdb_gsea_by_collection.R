...
BASE_OUTPUT_DIR <- "output/msigdb_by_collection"
...
cat(sprintf("\n✓ Loaded %d gene sets\n", initial_count))
cat(sprintf("✓ After size filtering (%d-%d genes): %d gene sets\n",
...
cat(sprintf("Testing %d gene sets with %d permutations...\n",
...
for (collection in sort(collections_in_results)) {
  ...
  if (nrow(collection_sig) > 0) {
    ...
    top_up <- collection_sig %>% filter(NES > 0) %>% arrange(desc(NES)) %>% head(10)
    for (i in 1:nrow(top_up)) {
      cat(sprintf("  %2d. %s (NES=%.2f, FDR=%.2e)\n",
                  i, top_up$pathway[i], top_up$NES[i], top_up$padj[i]),
          file = summary_file, append = TRUE)
    }

    cat("\nTOP 10 DOWNREGULATED:\n", file = summary_file, append = TRUE)
    top_down <- collection_sig %>% filter(NES < 0) %>% arrange(NES) %>% head(10)
