...
dir.create("output/only_degs/results/direct_targets", showWarnings = FALSE, recursive = TRUE)
dir.create("output/only_degs/results/pathway_analysis", showWarnings = FALSE, recursive = TRUE)
dir.create("output/only_degs/results/regulatory_networks", showWarnings = FALSE, recursive = TRUE)
dir.create("output/only_degs/plots", showWarnings = FALSE, recursive = TRUE)
...
write.csv(tes_direct_targets, "output/only_degs/results/direct_targets/TES_direct_targets.csv", row.names = FALSE)
write.csv(tead1_direct_targets, "output/only_degs/results/direct_targets/TEAD1_direct_targets.csv", row.names = FALSE)
write.csv(tes_specific, "output/only_degs/results/direct_targets/TES_specific_targets.csv", row.names = FALSE)
write.csv(tead1_specific, "output/only_degs/results/direct_targets/TEAD1_specific_targets.csv", row.names = FALSE)
...
write.csv(classification, "output/only_degs/results/regulatory_networks/gene_classification.csv", row.names = FALSE)
...
perform_enrichment <- function(gene_list, background, analysis_name) {
  ...
  if(nrow(ego) > 0) {
    ...
    write.csv(ego@result, sprintf("output/only_degs/results/pathway_analysis/%s_GO_enrichment.csv", analysis_name), row.names = FALSE)
    return(ego)
  } else {
...
pdf("output/only_degs/plots/01_gene_classification_pie.pdf", width = 10, height = 8)
...
pdf("output/only_degs/plots/02_expression_histograms.pdf", width = 14, height = 10)
...
pdf("output/only_degs/plots/03_expression_boxplots.pdf", width = 12, height = 8)
...
pdf("output/only_degs/plots/04_target_overlap_venn.pdf", width = 10, height = 8)
grid.newpage()
...
pdf("output/only_degs/plots/05_summary_barchart.pdf", width = 12, height = 8)
...
if(!is.null(tes_go) && nrow(tes_go) > 0) {
  pdf("output/only_degs/plots/TES_pathway_enrichment.pdf", width = 14, height = 10)
...
if(!is.null(tead1_go) && nrow(tead1_go) > 0) {
  pdf("output/only_degs/plots/TEAD1_pathway_enrichment.pdf", width = 14, height = 10)
...
if(!is.null(shared_go) && nrow(shared_go) > 0) {
  pdf("output/only_degs/plots/Shared_pathway_enrichment.pdf", width = 14, height = 10)
...
writeLines(c(
  ...
  sprintf("  TEAD1 peaks: %d total, %d with Ensembl ID, %d bound genes",
  ...
  "",
  "REGULATORY MODES:",
  sprintf("  TES: %d upregulated, %d downregulated (%.1f%% activation)",
  ...
  sprintf("  TEAD1: %d upregulated, %d downregulated (%.1f%% activation)",
...
), "output/only_degs/results/FINAL_ANALYSIS_SUMMARY.txt")
...
save(rna_results, rna_all, rna_significant_list, tes_peaks, tead1_peaks,
     ...
     file = "output/only_degs/results/FINAL_ANALYSIS_RESULTS.RData")

cat("\n✓ Complete analysis results saved to output/only_degs/results/FINAL_ANALYSIS_RESULTS.RData\n")
cat("✓ Summary report saved to output/only_degs/results/FINAL_ANALYSIS_SUMMARY.txt\n")

cat("\n" , rep("=", 80), "\n")
cat("INTEGRATIVE ANALYSIS COMPLETED SUCCESSFULLY!\n")
cat("Analysis finished:", as.character(Sys.time()), "\n")
cat("Results available in: output/only_degs/results/\n")
cat("Plots available in: output/only_degs/plots/\n")
cat(rep("=", 80), "\n")