...
# Output directories
output_dir <- "output/true_gsea_analysis"
...
fgsea_results <- read.csv(file.path(output_dir, "fgsea_all_pathways.csv"))
fgsea_sig <- read.csv(file.path(output_dir, "fgsea_significant_pathways.csv"))
fgsea_cancer <- read.csv(file.path(output_dir, "fgsea_cancer_pathways.csv"))

cat(sprintf("Loaded %d total pathways, %d significant\n",
...
if (nrow(fgsea_sig) > 0) {
  ...
  p1 <- ggplot(fgsea_sig, aes(x = NES, y = -log10(padj))) +
    ...
    geom_point(data = subset(fgsea_sig, label != ""),
    ...
    labs(title = "GSEA Enrichment Landscape",
         subtitle = "Top 20 pathways labeled | Dashed lines: |NES| = 1.5, FDR = 0.01",
         x = "Normalized Enrichment Score (NES)",
         y = "-log10(Adjusted p-value)") +
    theme_publication()

  ggsave(file.path(enhanced_dir, "01_volcano_plot.pdf"), p1,
         width = 14, height = 10, device = cairo_pdf)
  ggsave(file.path(enhanced_dir, "01_volcano_plot.png"), p1,
...
if (nrow(fgsea_cancer) > 0) {
  ...
  ggsave(file.path(enhanced_dir, "02_lollipop_plot.pdf"), p2,
         width = 14, height = 12, device = cairo_pdf)
  ggsave(file.path(enhanced_dir, "02_lollipop_plot.png"), p2,
...
if (nrow(fgsea_cancer) > 0 && !all(is.na(fgsea_cancer$category))) {
  ...
  ha_col <- HeatmapAnnotation(
    Metric = colnames(mat),
    col = list(Metric = c("Mean NES" = "#E41A1C",
                          "Median NES" = "#377EB8",
  ...
  pdf(file.path(enhanced_dir, "03_category_heatmap.pdf"), width = 12, height = 8)
  ht <- Heatmap(mat,
                name = "Score",
                top_annotation = ha_col,
                right_annotation = ha_row,
                col = colorRamp2(c(min(mat), 0, max(mat)),
  ...
  png(file.path(enhanced_dir, "03_category_heatmap.png"),
...
if (nrow(fgsea_cancer) > 0 && !all(is.na(fgsea_cancer$category))) {
  p4 <- ggplot(fgsea_cancer %>% filter(!is.na(category)),
    ...
    labs(title = "Pathway Enrichment Distribution by Category",
  ...
  ggsave(file.path(enhanced_dir, "04_ridge_plot.pdf"), p4,
         width = 12, height = 10, device = cairo_pdf)
  ggsave(file.path(enhanced_dir, "04_ridge_plot.png"), p4,
...
if (nrow(fgsea_sig) > 0 && nrow(fgsea_cancer) > 0) {
  ...
  summary_data <- data.frame(
    Category = c("Total Tested", "Significant (FDR<0.05)", "Cancer-Relevant",
                 "Upregulated (NES>0)", "Downregulated (NES<0)"),
    Count = c(nrow(fgsea_results), nrow(fgsea_sig), nrow(fgsea_cancer),
  ...
  top10_up <- fgsea_sig %>% filter(NES > 0) %>% arrange(desc(NES)) %>% head(10)
  top10_down <- fgsea_sig %>% filter(NES < 0) %>% arrange(NES) %>% head(10)
  top10 <- rbind(top10_up, top10_down) %>%
    mutate(pathway_label = substr(pathway, 1, 40),
  ...
  pC <- ggplot(fgsea_sig, aes(x = -log10(padj))) +
    ...
    annotate("text", x = -log10(0.05), y = Inf, label = "FDR = 0.05",
             vjust = 2, hjust = -0.1, color = "red", fontface = "bold") +
    labs(title = "C) Significance Distribution",
  ...
  pD <- ggplot(fgsea_sig, aes(x = size, y = NES, color = -log10(padj))) +
    ...
    labs(title = "D) Gene Set Size vs Enrichment",
  ...
  combined <- (pA | pB) / (pC | pD) +
    plot_annotation(
      ...
      subtitle = sprintf("Total: %d pathways tested | %d significant (FDR < 0.05)",
                        nrow(fgsea_results), nrow(fgsea_sig)),
      theme = theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
                    plot.subtitle = element_text(size = 12, hjust = 0.5))
    )

  ggsave(file.path(enhanced_dir, "05_multipanel_summary.pdf"), combined,
         width = 18, height = 12, device = cairo_pdf)
  ggsave(file.path(enhanced_dir, "05_multipanel_summary.png"), combined,
