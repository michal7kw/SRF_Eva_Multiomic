...
# Output directory
enhanced_dir <- "output/enhanced_integrative_plots"
...
gene_class <- read.csv("output/results/regulatory_networks/gene_classification_all_genes.csv")
...
combined_data <- gene_class %>%
  ...
  mutate(
    ...
    is_significant = is_significant,  # Use existing column
    quadrant = case_when(
...
p1 <- ggplot(combined_data, aes(x = log2FoldChange, y = -log10(padj))) +
  # Background points
  geom_point(data = subset(combined_data, binding_category == "No binding"),
  ...
  scale_color_manual(values = c("Both" = "#E41A1C",
                                 ...
                                 "No binding" = "grey70"),
                     name = "Binding") +
  scale_size_manual(values = c("Both" = 3, "TES only" = 2.5,
                                "TEAD1 only" = 2.5, "No binding" = 1),
                    guide = "none") +
  labs(title = "Integrative Analysis: TF Binding vs Gene Expression",
       subtitle = "Significant direct targets labeled (FDR < 0.05, |log2FC| > 1)",
       x = "log2 Fold Change (TES vs GFP)",
       y = "-log10(Adjusted p-value)") +
  theme_pub()

ggsave(file.path(enhanced_dir, "01_quadrant_binding_expression.pdf"), p1,
       width = 14, height = 10, device = cairo_pdf)
ggsave(file.path(enhanced_dir, "01_quadrant_binding_expression.png"), p1,
...
p2 <- ggplot(overlap_data, aes(x = Category, y = Count, fill = Category)) +
  ...
  scale_fill_manual(values = c("TES only" = "#E41A1C",
                                "Shared" = "#984EA3",
                                "TEAD1 only" = "#377EB8")) +
  labs(title = "TES vs TEAD1 Direct Target Overlap",
       subtitle = sprintf("Total: %d TES targets, %d TEAD1 targets",
...
ggsave(file.path(enhanced_dir, "02_overlap_TES_TEAD1.pdf"), p2,
       width = 10, height = 8, device = cairo_pdf)
ggsave(file.path(enhanced_dir, "02_overlap_TES_TEAD1.png"), p2,
...
pdf(file.path(enhanced_dir, "03_top_direct_targets_heatmap.pdf"), width = 10, height = 14)
ht <- Heatmap(mat,
              name = "log2FC",
...
png(file.path(enhanced_dir, "03_top_direct_targets_heatmap.png"),
...
p4 <- ggplot(category_counts, aes(x = reorder(category, count), y = count, fill = direction)) +
  ...
  scale_fill_manual(values = c("UP" = "#E74C3C", "DOWN" = "#3498DB"),
                    name = "Direction") +
  coord_flip() +
  labs(title = "Gene Regulatory Categories",
       subtitle = "Classification based on binding and expression",
       x = NULL,
       y = "Number of Genes") +
  theme_pub()

ggsave(file.path(enhanced_dir, "04_target_categories.pdf"), p4,
       width = 12, height = 7, device = cairo_pdf)
ggsave(file.path(enhanced_dir, "04_target_categories.png"), p4,
...
pC <- ggplot(overlap_data, aes(x = Category, y = Count, fill = Category)) +
  ...
  scale_fill_manual(values = c("TES only" = "#E41A1C",
                                "Shared" = "#984EA3",
                                "TEAD1 only" = "#377EB8")) +
  labs(title = "C) TES vs TEAD1 Overlap",
       x = NULL,
       y = "Direct Targets") +
  theme_pub() +
  theme(legend.position = "none") +
...
pD <- ggplot(fc_dist, aes(x = log2FoldChange, fill = category)) +
  ...
  scale_fill_manual(values = c("Both" = "#E41A1C", "TES only" = "#377EB8",
                                "TEAD1 only" = "#4DAF4A", "Indirect" = "#984EA3"),
                    name = "Category") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  labs(title = "D) Expression Change Distribution",
       x = "log2 Fold Change",
       y = "Density") +
  theme_pub()
...
ggsave(file.path(enhanced_dir, "05_multipanel_integrative_summary.pdf"), combined_summary,
       width = 16, height = 12, device = cairo_pdf)
ggsave(file.path(enhanced_dir, "05_multipanel_integrative_summary.png"), combined_summary,
...
cat("  2. 02_upset_gene_intersections.pdf/png\n")
