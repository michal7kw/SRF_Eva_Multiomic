...
output_dir <- "output/gsea_cancer_pathways"
...
tes_direct <- read.csv("output/results/direct_targets/TES_direct_targets_all_genes.csv",
                       stringsAsFactors = FALSE)
tead1_direct <- read.csv("output/results/direct_targets/TEAD1_direct_targets_all_genes.csv",
                         stringsAsFactors = FALSE)
tes_specific <- read.csv("output/results/direct_targets/TES_specific_targets_all_genes.csv",
                         stringsAsFactors = FALSE)
tead1_specific <- read.csv("output/results/direct_targets/TEAD1_specific_targets_all_genes.csv",
                           stringsAsFactors = FALSE)
...
tes_go <- read.csv("output/results/pathway_analysis/TES_direct_GO_enrichment_all_genes.csv",
                   stringsAsFactors = FALSE)
tead1_go <- read.csv("output/results/pathway_analysis/TEAD1_direct_GO_enrichment_all_genes.csv",
                     stringsAsFactors = FALSE)
shared_go <- read.csv("output/results/pathway_analysis/TES_TEAD1_shared_GO_enrichment_all_genes.csv",
                      stringsAsFactors = FALSE)
...
cancer_keywords <- list(
  apoptosis = c("apoptosis", "apoptotic", "cell death", "programmed cell death",
                "necrosis", "necrotic", "ferroptosis", "pyroptosis", "anoikis"),
  migration = c("migration", "migratory", "motility", "invasion", "invasive",
                "chemotaxis", "chemotactic", "cell movement", "locomotion"),
  proliferation = c("proliferation", "proliferative", "cell cycle", "mitosis",
...
cat(sprintf("✓ TES cancer pathways: %d (%d apoptosis, %d migration, %d proliferation)\n",
            nrow(tes_cancer),
...
cat(sprintf("✓ TEAD1 cancer pathways: %d (%d apoptosis, %d migration, %d proliferation)\n",
            nrow(tead1_cancer),
...
cat(sprintf("✓ Shared cancer pathways: %d (%d apoptosis, %d migration, %d proliferation)\n\n",
            nrow(shared_cancer),
...
all_cancer_pathways <- rbind(
  tes_cancer[, c("ID", "Description", "GeneRatio", "pvalue", "p.adjust",
                 "qvalue", "Count", "pathway_category", "target_group")],
  tead1_cancer[, c("ID", "Description", "GeneRatio", "pvalue", "p.adjust",
                   "qvalue", "Count", "pathway_category", "target_group")],
  shared_cancer[, c("ID", "Description", "GeneRatio", "pvalue", "p.adjust",
                    "qvalue", "Count", "pathway_category", "target_group")]
)

# Parse GeneRatio for plotting
all_cancer_pathways$GeneRatio_numeric <- sapply(strsplit(all_cancer_pathways$GeneRatio, "/"),
                                                 function(x) as.numeric(x[1])/as.numeric(x[2]))

cat(sprintf("✓ Total cancer-relevant pathway entries: %d\n", nrow(all_cancer_pathways)))
cat(sprintf("✓ Unique pathways across all groups: %d\n\n",
...
p1 <- ggplot(category_counts, aes(x = pathway_category, y = count, fill = target_group)) +
  ...
  labs(title = "Cancer-Relevant Pathway Enrichment by Target Group",
...
p2 <- ggplot(top_pathways, aes(x = target_group, y = Description,
  ...
  labs(title = "Top 10 Cancer Pathways per Category and Target Group",
       x = "Target Group",
...
pathway_matrix <- top_pathways_for_heatmap %>%
  ...
  tidyr::pivot_wider(names_from = target_group, values_from = p.adjust,
                     values_fill = 1) %>%
  as.data.frame()
...
p5 <- ggplot(all_cancer_pathways %>% filter(!is.na(pathway_category)),
  ...
  labs(title = "Distribution of Gene Counts in Cancer Pathways",
...
write.table(summary_stats, file = summary_file, append = TRUE,
...
cat(sprintf("  Apoptosis: %.1f%%, Migration: %.1f%%, Proliferation: %.1f%%\n\n",
