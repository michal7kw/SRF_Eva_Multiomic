...
for (i in seq_along(gmt_lines)) {
  tryCatch({
    ...
    if (length(parts) < 3) {
      cat(sprintf("  [%d/%d] SKIP: %s (too few columns)\n",
    ...
    if (length(genes) == 0) {
      cat(sprintf("  [%d/%d] SKIP: %s (no genes)\n",
...
cat("  sbatch scripts/run_msigdb_gsea.sh\n\n")
