...
#SBATCH --output=logs/advanced_phase3_1_methylation_binding.out
#SBATCH --error=logs/advanced_phase3_1_methylation_binding.err

echo "=================================================="
echo "Phase 3.1: Methylation at TES vs TEAD1 Binding Sites"
...
MEDIP_BIGWIGS="meDIP/results/05_bigwig"
...
# Check if binding classification exists
if [ ! -d "${INPUT_DIR}" ]; then
    echo "ERROR: Binding classification not found!"
    echo "Please run Phase 1.1 first"
    exit 1
fi

# Check if meDIP BigWigs exist
if [ ! -d "${MEDIP_BIGWIGS}" ]; then
    echo "ERROR: meDIP BigWig files not found!"
    echo "Expected location: ${MEDIP_BIGWIGS}"
    echo "Please ensure meDIP pipeline has generated BigWig files"
    exit 1
fi

# List available BigWig files
echo "Available meDIP BigWig files:"
ls -lh ${MEDIP_BIGWIGS}/*RPKM.bw
...
for BW in ${GFP_BIGWIGS} ${TES_BIGWIGS}; do
    if [ ! -f "${BW}" ]; then
        echo "WARNING: ${BW} not found, skipping..."
    fi
done
...
computeMatrix reference-point \
    ...
    -S ${GFP_BIGWIGS} ${TES_BIGWIGS} \
    -R ${REGION_FILES} \
    -a 5000 -b 5000 \
    --binSize 50 \
    --missingDataAsZero \
    --numberOfProcessors 16 \
    -o ${OUTPUT_DIR}/methylation_at_binding_sites.mat.gz

echo ""
echo "Step 3: Creating heatmaps..."
echo ""

# Heatmap: Methylation at TES and TEAD1 sites
plotHeatmap \
    -m ${OUTPUT_DIR}/methylation_at_binding_sites.mat.gz \
    -o ${OUTPUT_DIR}/methylation_at_TES_TEAD1_sites.pdf \
    --colorMap YlOrRd \
...
plotProfile \
    -m ${OUTPUT_DIR}/methylation_at_binding_sites.mat.gz \
    -o ${OUTPUT_DIR}/methylation_profile_by_category.pdf \
    --perGroup \
    --plotHeight 12 \
    --plotWidth 15 \
    --refPointLabel "Peak Center" \
...
# Run R script
Rscript ${OUTPUT_DIR}/quantify_methylation.R

echo ""
echo "=================================================="
echo "Methylation-binding analysis complete: $(date)"
echo "=================================================="
echo ""
echo "Results location: ${OUTPUT_DIR}"
echo ""
echo "Key outputs:"
echo "  - methylation_at_TES_TEAD1_sites.pdf"
echo "  - methylation_profile_by_category.pdf"
echo "  - dmr_overlap_by_category.csv"
echo ""
echo "Next steps:"
echo "  1. Review heatmaps and profile plots"
echo "  2. Check DMR overlap statistics"
echo "  3. Proceed to Phase 3.2 (Promoter Methylation-Expression)"
