...
JOB1=$(sbatch --parsable scripts/run_true_gsea.sh)
...
cd ${BASE_DIR}/SRF_Eva_integrated_analysis
JOB3=$(sbatch --parsable scripts/candidate_gene_heatmap.sh)
...
if [ -f "${BASE_DIR}/SRF_Eva_integrated_analysis/output/results/direct_targets/TES_direct_targets_all_genes.csv" ]; then
    ...
    echo "  Please run integrative analysis first: sbatch SRF_Eva_integrated_analysis/scripts/run_all_genes_analysis.sh"
    JOB4=""
fi
echo ""
