...
dir.create("results/direct_targets", showWarnings = FALSE, recursive = TRUE)
dir.create("results/pathway_analysis", showWarnings = FALSE, recursive = TRUE)
dir.create("results/regulatory_networks", showWarnings = FALSE, recursive = TRUE)
...
write.csv(tes_direct_targets, "results/direct_targets/TES_direct_targets.csv", row.names = FALSE)
write.csv(tead1_direct_targets, "results/direct_targets/TEAD1_direct_targets.csv", row.names = FALSE)
write.csv(tes_specific, "results/direct_targets/TES_specific_targets.csv", row.names = FALSE)
write.csv(tead1_specific, "results/direct_targets/TEAD1_specific_targets.csv", row.names = FALSE)
...
write.csv(classification, "results/regulatory_networks/gene_classification.csv", row.names = FALSE)
...
perform_enrichment <- function(gene_list, background, analysis_name) {
  ...
  if(nrow(ego) > 0) {
    ...
    write.csv(ego@result, sprintf("results/pathway_analysis/%s_GO_enrichment.csv", analysis_name), row.names = FALSE)
    return(ego)
  } else {
...
if(!is.null(tes_go) && nrow(tes_go) > 0) {
  pdf("plots/TES_pathway_enrichment.pdf", width = 14, height = 10)
...
if(!is.null(tead1_go) && nrow(tead1_go) > 0) {
  pdf("plots/TEAD1_pathway_enrichment.pdf", width = 14, height = 10)
...
if(!is.null(shared_go) && nrow(shared_go) > 0) {
  pdf("plots/Shared_pathway_enrichment.pdf", width = 14, height = 10)
...
save(rna_results, rna_all, rna_significant_list, tes_peaks, tead1_peaks,
     ...
     file = "results/FINAL_ANALYSIS_RESULTS.RData")

cat("\n✓ Complete analysis results saved to results/FINAL_ANALYSIS_RESULTS.RData\n")
cat("✓ Summary report saved to results/FINAL_ANALYSIS_SUMMARY.txt\n")
...
cat("Plots available in: integrative_analysis/plots/\n")
cat(rep("=", 80), "\n")