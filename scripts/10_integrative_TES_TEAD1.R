...
################################################################################
# Script: 10_integrative_TES_TEAD1.R
...
out_dir <- file.path(base_dir, "SRF_Eva_Multiomic/scripts/results/10_integrative_TES_TEAD1")
...
if (nrow(direct_targets) >= 5) {
    ...
    heatmap_mat <- data.frame(
        ...
        TEAD1_binding = ifelse(top_targets$has_TEAD1_binding, 1, 0),
    ...
    pdf("TES_TEAD1_direct_epigenetic_targets_heatmap.pdf", width = 10, height = 12)
    pheatmap(as.matrix(heatmap_mat),
        cluster_cols = FALSE,
        cluster_rows = TRUE,
        show_rownames = TRUE,
        color = colorRampPalette(c("blue", "white", "red"))(100),
        main = "Direct Epigenetic Targets - TES vs TEAD1 (Top 50)",
        fontsize_row = 6,
        angle_col = 45
    )
    dev.off()
    cat("Saved: TES_TEAD1_direct_epigenetic_targets_heatmap.pdf\n\n")
}
...
cat("  5. TES_TEAD1_direct_epigenetic_targets_heatmap.pdf - Heatmap of top targets\n")
cat("  6. TES_TEAD1_integration_summary.txt - Statistical summary\n\n")
