...
OUTPUT_DIR <- file.path(BASE_DIR, "SRF_Eva_integrated_analysis/output/density_plots")
...
create_signal_matrix <- function(bw_file, regions, upstream, downstream, window_size) {
  ...
  cat("    Score range:", min(coverage$score, na.rm = TRUE), "to",
  ...
  cat("    Matrix range before cleanup:", min(mat, na.rm = TRUE), "to",
      max(mat, na.rm = TRUE), "\n")
...
if (data_max > data_min && data_max > 0) {
  ...
  col_signal <- colorRamp2(c(0, data_median, data_max),
                          c("white", "yellow", "red"))
...
for (sample_name in c("TES_combined", "TEAD1_combined", "TESmut_combined")) {
  if (sample_name %in% names(signal_matrices)) {
    ...
    ht <- EnrichedHeatmap(mat_full,
                          ...
                          axis_name = c(paste0("-", UPSTREAM/1000, "kb"), "TSS",
                          ...
                          heatmap_height = unit(0.8, "npc"))  # Set explicit height
...
plot(NULL, xlim = c(-UPSTREAM, DOWNSTREAM), ylim = c(0, max(unlist(profiles), na.rm = TRUE)),
...
plot(x_coords, profile_up, type = "l", col = "red", lwd = 2,
...
tryCatch({
  draw(gene_anno + ht_list, heatmap_legend_side = "right",
